...
test.describe('Search and filter products', () => {
  test('should filter products by category when typing in search', async ({ page }) => {
    ...
    // 2. Type 'kitchen' into the search input (aria-label "Search products")
    await page.getByRole('textbox', { name: 'Search products' }).fill('kitchen');
